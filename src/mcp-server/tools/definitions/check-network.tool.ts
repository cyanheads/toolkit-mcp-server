...
export const checkNetworkTool = tool('toolkit_check_network', {
  ...
  output: z.object({
    ...
    rttMs: z
      .number()
      .optional()
      .describe('Average round-trip time in milliseconds. Present for ping when reachable.'),
    hops: z
      .array(
        z
          .object({
  ...
  format: (result) => {
    ...
    if (result.reachable !== undefined) {
      ...
      const rtt = result.rttMs != null ? ` — avg ${result.rttMs} ms` : '';
      lines.push(`**${result.target ?? '?'}:** ${reach}${rtt}`);
    }
